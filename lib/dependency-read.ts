...
/**
 * A deliberately conservative analyst read. Every affirmative statement comes from
 * retained graph edges, their explicitly stated percentages, or a report key-risk.
 */
export function getDependencyRead(graph: GraphData, meta?: AnalysisMeta): DependencyRead {
  ...
  const distinctCustomers = new Set(
    customerExposureEdges
      .map((edge) => getTargetCounterparty(edge, targetId, nodeById)?.id)
  ...
  const evidenceCoverage = { retained: graph.edges.length, excluded: meta?.excluded.length ?? null };
  ...
  const concentrationLine: DependencyReadLine = customerExposureEdges.length > 0
    ? {
        ...
        text: `≥${formatPercentage(customerConcentration)}% explicitly quantified across ${distinctCustomers.size} ${distinctCustomers.size === 1 ? "customer" : "customers"}`,
        tone: customerExposureEdges.some((edge) => edge.risk_flag === "high") ? "high" : "medium",
        edges: customerExposureEdges,
        nodeIds: customerNodeIds,
      }
    : {
        id: "customer_concentration",
        label: "Revenue exposure",
        text: "No explicit customer percentage retained",
        tone: "low",
        edges: [],
        nodeIds: [],
      };
  ...
  const singlePointLine: DependencyReadLine = singlePoints.length > 0
    ? {
        ...
        label: "Single points",
        text: `${singlePoints.length} ${singlePoints.length === 1 ? "counterparty" : "counterparties"} · ${summariseNames(singlePoints.map(({ node, edge }) => `${node.label}${typeof edge.exposure_pct === "number" ? ` (${formatPercentage(edge.exposure_pct)}%)` : " (high risk)"}`))}`,
        tone: singlePoints.some(({ edge }) => edge.risk_flag === "high") ? "high" : "medium",
        edges: singlePointEdges,
        nodeIds: singlePoints.map(({ node }) => node.id),
      }
    : {
        id: "single_points_of_failure",
        label: "Single points",
        text: "No retained counterparties meet the 25% / high-risk threshold",
        tone: "low",
        edges: [],
        nodeIds: [],
      };
  ...
  const watchLine: DependencyReadLine = watchItems.length > 0
    ? {
        id: "watch_items",
        label: "Watch items",
        text: summariseNames(watchItems.map((item) => item.text)),
        tone: watchEdges.some((edge) => edge.risk_flag === "high") ? "high" : "medium",
        edges: watchEdges,
        nodeIds: watchNodeIds,
      }
    : {
        id: "watch_items",
        label: "Watch items",
        text: "No unnamed or key-risk dependencies retained",
        tone: "low",
        edges: [],
        nodeIds: [],
      };

  const evidenceLine: DependencyReadLine = {
    id: "evidence_coverage",
    label: "Evidence coverage",
    text: `${evidenceCoverage.retained} retained relationship${evidenceCoverage.retained === 1 ? "" : "s"}${evidenceCoverage.excluded === null ? "" : ` · ${evidenceCoverage.excluded} excluded`}`,
    tone: evidenceCoverage.excluded && evidenceCoverage.excluded > 0 ? "medium" : "low",
    edges: [],
    nodeIds: [],
  };
  ...
  const tone = singlePoints.some(({ edge }) => edge.risk_flag === "high") || customerExposureEdges.some((edge) => edge.risk_flag === "high")
    ? "high"
    : customerExposureEdges.length > 0 || watchItems.length > 0
      ? "medium"
      : "low";
  ...
  return {
    ...
    single_points_of_failure: singlePoints,
    watch_items: watchItems,
    evidence_coverage: evidenceCoverage,
    lines: [concentrationLine, singlePointLine, watchLine, evidenceLine],
  };
}
